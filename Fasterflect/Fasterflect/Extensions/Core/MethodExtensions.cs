...
namespace Fasterflect
{
    ...
    public static class MethodExtensions
    {
        ...
        /// <returns>The specified method or null if no method was found. If there are multiple matches
        /// due to method overloading the first found match will be returned.</returns>
        public static MethodInfo Method( this Type type, string name, Flags bindingFlags, Type[] paramTypes )
        {
            ...
            {
                return type.Methods( paramTypes, bindingFlags, name ).FirstOrDefault();
            }

            bool hasTypes = paramTypes != null;
            var result = hasTypes
                             ? type.GetMethod( name, bindingFlags, null, paramTypes, null )
                             : type.GetMethod( name, bindingFlags );
            if( result == null && bindingFlags.IsNotSet( Flags.DeclaredOnly ) )
            {
                if( type.BaseType != typeof(object) && type.BaseType != null )
                {
                    return type.BaseType.Method( name, bindingFlags, paramTypes );
                }
            }
