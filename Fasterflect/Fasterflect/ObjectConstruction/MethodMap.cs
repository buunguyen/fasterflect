		...
		private void InitializeMethodMap( object[] sampleParamValues )
		{
			...
			for( int invokeParamIndex = 0; invokeParamIndex < paramNames.Length; invokeParamIndex++ )
			{
				...
				if( ! foundParam )
				{
					...
					MemberInfo member = type.Property( paramName, Flags.InstanceCriteria | Flags.IgnoreCase );
					member = member != null && member.CanWrite() ? member : type.Field( paramName, Flags.InstanceCriteria | Flags.IgnoreCase );
