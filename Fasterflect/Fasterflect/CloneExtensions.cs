...
namespace Fasterflect
{
	...
	public static class CloneExtensions
	{
		...
		public static T DeepClone<T>(this T source) where T : class, new()
		{
			...
			object[] values = fields.Select(f => GetValue(f, source, clone)).ToArray();
		...
		private static object GetValue(FieldInfo field, object source, object clone)
		{
			object result = field.GetValue(source);
			bool cyclic = ReferenceEquals(source, result);
			bool follow = result != null && result.GetType().IsClass && result.GetType() != typeof (string);
			return cyclic ? clone : follow ? result.DeepClone() : result;
		}
